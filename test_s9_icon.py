...
async def run():
    async with async_playwright() as p:
        ...
        # Try to click the MOVIE ICON of an incomplete lecture
        lectures = await page.query_selector_all('#contentsPropertyList .rw.data2')
        target_icon = None
        for lecture in lectures:
            comp = await lecture.get_attribute('data-complete')
            if comp != 'on':
                target_icon = await lecture.query_selector('.play.movie .icon')
                if target_icon:
                    break
        
        if target_icon:
            print("Clicking movie icon...")
            await human_move_and_click(page, target_icon)
            ...
            if await page.is_visible('#mPlayer:not(.hidden)'):
                ...
                await page.screenshot(path="screenshots/s9_icon_fail.png")
        else:
            print("No incomplete movie icon found in S9.")
